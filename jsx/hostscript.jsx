/*jslint vars: true, plusplus: true, devel: true, nomen: true, regexp: true, indent: 4, maxerr: 50 */
/*global $, Folder*/
#include './storage.jsx';

function saves(namb) {
    var n = "" + namb;
	//alert(n);
    var pathNamb = "\\\\storage\\zakaz\\" + n.substring(0, n.length - 3) + "000-" + n.substring(0, n.length - 3) + "999\\" + n + "\\maket\\" + n;
    //alert(pathNamb);
	var saveOptions = new IllustratorSaveOptions();
    saveOptions.compatibility = Compatibility.ILLUSTRATOR17;
    app.activeDocument.saveAs(new File(pathNamb), saveOptions);
    
    if (activeDocument.layers["Knife"].groupItems.length) {
        var gr0 = activeDocument.layers["Knife"].groupItems[0];
        while (gr0.pathItems.length == 0) {
            gr0 = gr0.groupItems[0];
        }
        if (Math.round(gr0.pathItems[0].strokeColor.yellow) != 100)
            alert("Проверить цветность элементов, нет 100% цвета.");
    } else {
        if (activeDocument.layers["Knife"].pathItems.length > 0) {
            for (var i = 0; i < 20; i++) {
                if (activeDocument.layers["Knife"].pathItems[i].strokeColor.yellow != 1) {
                    if (Math.round(activeDocument.layers["Knife"].pathItems[i].strokeColor.yellow) != 100) {
                        alert("Проверить цветность элементов, нет 100% цвета");
                        break;
                    } else break;
                }
            }
        } else alert("Невозможно проверить цветность.");
    }
    return;
}

function sayHello(sp) {
    var Nams = "" + (sp.Namber || sp.Namb);
    var Rap = sp.Raport;
    var irll = sp.irll;
    const mm = 72.0 / 25.4;
    var PolurotY = sp.PolurotY;
    var Polurot = sp.Polurot;

    $.global.Tolerance = (sp.Tolerance !== undefined && sp.Tolerance !== '' && parseFloat(sp.Tolerance) > 0) ? parseFloat(sp.Tolerance) : 0.5;
    $.global.DistX = (sp.DistX !== undefined && sp.DistX !== '' && !isNaN(parseFloat(sp.DistX))) ? parseFloat(sp.DistX) : 100;
    $.global.DistShear = (sp.DistShear !== undefined && sp.DistShear !== '' && !isNaN(parseFloat(sp.DistShear))) ? parseFloat(sp.DistShear) : 0;
    $.global.DistShearY = (sp.DistShearY !== undefined && sp.DistShearY !== '' && !isNaN(parseFloat(sp.DistShearY))) ? parseFloat(sp.DistShearY) : 0;
    
    if (sp.btnRll) {
        runExpDXF(Nams, Polurot || Rap, mm, irll);
        return;
    }

    var fonts = "MagistralTT";
    var docRef = app.activeDocument;
    var newLayer = docRef.activeLayer;
    newLayer.name = "Knife";
  
    var L_Test = docRef.layers.add();
    L_Test.zOrder(ZOrderMethod.SENDTOBACK);
    L_Test.name = "L-Test";
	
    var Linfo = docRef.layers.add();
    Linfo.zOrder(ZOrderMethod.SENDTOBACK);
    Linfo.name = "info";
    
    var REZ = docRef.layers.add();
    REZ.zOrder(ZOrderMethod.SENDTOBACK);
    REZ.name = "REZ";
    
    var NambeL = docRef.layers.add();
    NambeL.zOrder(ZOrderMethod.SENDTOBACK);
    NambeL.name = "namber";
    NambeL.printable = false;
    
    docRef.rulerOrigin = Array(0, 0);
    var units = 1;
    app.preferences.setIntegerPreference("rulerType", units);
    app.preferences.setIntegerPreference("strokeUnits", units);
    
    var wi = docRef.width;
    var hi = docRef.height;
    var Da = new Date();
    var Dat = String('' + Da.getDate() + '-' + (Da.getMonth() + 1) + '-' + Da.getFullYear());
    
    var dop = '';
    var rll = '', raa = '';
    var Nam = sp.Namber;
    var Cust = sp.Customer;
    if (sp.rll != '') { dop = rll = ' / ' + sp.rll; }
    if (sp.raa != '') { dop += raa = ' / ' + sp.raa; }
    if (sp.rez != '') { dop += ' / ' + sp.rez; }
    if (sp.perf != '') { dop += ' / ' + sp.perf; }
    if (sp.micro != '') { dop += ' / ' + sp.micro; }
    if (sp.google != '') { dop += ' / ' + sp.google; }
    
    var perimetr = 0;
    perimetr = sp.perimetr;
    var Rep = sp.Repetition;
    var Stre = sp.Streams;
    var Gapp = sp.GAP.split(' ');
    
    // === ОФСЕТ: +radX по X (вправо), +radY по Y (вверх) ===
	var radX = 0.0;
	var radY = 0.0;
	var ofst = 0.0;
	if (sp.rad) {
		radX = 3 * mm;
		radY = 5 * mm;
		ofst = 10.0 * mm;
	}


    var Gap2 = Gapp[1] * 1.0;
    var casi = sp.casing;
    var Distor = sp.Dis;
    var Ang = sp.Material.substring(0, sp.Material.indexOf(" "));
    var Mate = sp.Material.substring(sp.Material.indexOf(" "), sp.Material.lastIndexOf(" "));
    var Kni = sp.Knife;

    var SAll = sp.Forms.split(';');
	var Lval;
	Distor != 1 ? Lval = (Rap - 2.4) : Lval = "Нет";
    var Plosk = false;
    if (~Mate.indexOf("Плоская")) {Plosk = true; Rap = Polurot; Distor = 1.0;} 
    var lineOb = parseFloat(sp.Material.substring(sp.Material.lastIndexOf(" ")));
    var mms = mm / 2.0;
    var WidthLab = 19 * mm;
    var xEl = 0, yEl = 0;
    var compens = 6;
    var elm = newLayer.groupItems.add();
    var docSelection = new Array();
    var docSelection2 = new Array();

    var procText = sp.colorText, procDash = sp.colorDush, procRisk = sp.colorRisk;
    var dotPoint = 2.0;
    if (sp.gross) {
        procText = sp.colorText1;
        procDash = sp.colorDush1;
        procRisk = sp.colorRisk1;
        dotPoint = 3.0;
    }
    
    var Prll = new CMYKColor();
    Prll.name = 'rllColor';
    Prll.black = 0; Prll.cyan = 0; Prll.magenta = 0; Prll.yellow = 1;
    
    var PLabel = new CMYKColor();
    PLabel.name = 'labelColor';
    PLabel.black = 0; PLabel.cyan = 0; PLabel.magenta = procText; PLabel.yellow = 0;
    
    var PCyan = new CMYKColor();
    PCyan.name = 'labelColor';
    PCyan.black = 0; PCyan.cyan = 0; PCyan.magenta = procDash; PCyan.yellow = 0;
    
    var PRisk = new CMYKColor();
    PRisk.name = 'labelColor';
    PRisk.black = 0; PRisk.cyan = 0; PRisk.magenta = procRisk; PRisk.yellow = 0;
    
    var PWhite = new CMYKColor();
    PWhite.name = 'WhiteColor';
    PWhite.black = 0; PWhite.cyan = 0; PWhite.magenta = 0; PWhite.yellow = 0;
    
    var PFull = new CMYKColor();
    PFull.name = 'FullColor';
    PFull.black = 100; PFull.cyan = 100; PFull.magenta = 100; PFull.yellow = 100;
    
    if (casi == 2) { if (SAll[1] == 'true') { closePath(); } el(); Gap2 = Rap / Rep - SAll[1]; elSelect(0); recRap(Rap, (SAll[0] * Stre) + (Gapp[0] * Stre) + 15); return; }
    if (casi == 3) { if (SAll[1] == 'true') { closePath(); } el(); fullSelect(); }
    if (casi == 0) { circ(); elSelect(1); }
    if (casi == 1) { rect(); elSelect(1); }
    
    if (Polurot !== '') Rap = Polurot;
    if (~Mate.indexOf("Полуротация")) {
        if (Polurot === '0')
            if (casi == 3) Rap = elm.width / mm + 10;
            else Rap = (elm.width / mm + Gap2) * Rep - Gap2 + 10;
    }
    if (~Mate.indexOf("Ротация")) Polurot = Rap;

    var newGroup = newLayer.groupItems.add();
    for (var i = 0; i < docRef.pathItems.length; i++) {
        var a = docRef.pathItems[i];
        a.moveToBeginning(newGroup);
    }
    for (var i = 0; i < docRef.textFrames.length; i++) {
        var a = docRef.textFrames[i];
        a.moveToBeginning(newGroup);
    }
    
    var WidthForm = (SAll[0] * Stre) + (Gapp[0] * Stre) + 20 - compens;
    if (PolurotY !== '') WidthForm = PolurotY;
    
    if (PolurotY <= newGroup.height / mm + 0.5 && PolurotY !== '') alert("Нож не влезает в эту ширину!");
    if (Polurot <= newGroup.width / mm + 0.5) alert("Нож не влезает в эту длину!");
    
    var LI = Linfo.groupItems.add();
    var RiskGorizont = LI.groupItems.add();
    
    // === Oporka получает офсет +radX ===
    var Oporka = LI.pathItems.rectangle(12 * mm + radY, 5 * mm + radX, Rap * mm, -WidthForm * mm);
    Oporka.closed = true;
    Oporka.filled = false;
    Oporka.stroked = true;
    Oporka.strokeColor = PCyan;
    Oporka.fillOverprint = false;
    Oporka.strokeWidth = 0.1 * mm;
    Oporka.strokeDashes = [2, 2, 2, 2];
	
	if (sp.rad) {
		
	var OporkaOffset = Linfo.pathItems.rectangle(0, 0, (Rap * Distor) * mm + (2*ofst), (-WidthForm) * mm - (2*ofst));
		OporkaOffset.top = Oporka.top + ofst;
		OporkaOffset.left = Oporka.left - ofst;
		OporkaOffset.closed = true;
		OporkaOffset.filled = false;
		OporkaOffset.stroked = true;
		OporkaOffset.strokeColor = PCyan;
		OporkaOffset.fillOverprint = false;
		OporkaOffset.strokeWidth = 0.1 * mm;
		OporkaOffset.strokeDashes = [2, 2, 2, 2];
	}
    
    var angleLB = LI.pathItems.add();
    angleLB.setEntirePath(Array(Array(2 * mm + radX, 12 * mm + radY), Array(5 * mm + radX, 12 * mm + radY), Array(5 * mm + radX, 9 * mm + radY)));
    angleLB.stroked = true;
    angleLB.strokeColor = PCyan;
    angleLB.strokeWidth = 0.1 * mm;
    angleLB.filled = false;

    var angleLT = LI.pathItems.add();
    angleLT.setEntirePath(Array(Array(2 * mm + radX, WidthForm * mm + 12 * mm + radY), Array(5 * mm + radX, WidthForm * mm + 12 * mm + radY), Array(5 * mm + radX, WidthForm * mm + 15 * mm + radY)));
    angleLT.stroked = true;
    angleLT.strokeColor = PCyan;
    angleLT.strokeWidth = 0.1 * mm;
    angleLT.filled = false;
 
    var angleRT = LI.pathItems.add();
    angleRT.setEntirePath(Array(Array(Rap * mm + 8 * mm + radX, 12 * mm + radY), Array(Rap * mm + 5 * mm + radX, 12 * mm + radY), Array(Rap * mm + 5 * mm + radX, 9 * mm + radY)));
    angleRT.stroked = true;
    angleRT.strokeColor = PCyan;
    angleRT.strokeWidth = 0.1 * mm;
    angleRT.filled = false;

    var angleRB = LI.pathItems.add();
    angleRB.setEntirePath(Array(Array(Rap * mm + 8 * mm + radX, WidthForm * mm + 12 * mm + radY), Array(Rap * mm + 5 * mm + radX, WidthForm * mm + 12 * mm + radY), Array(Rap * mm + 5 * mm + radX, WidthForm * mm + 15 * mm + radY)));
    angleRB.stroked = true;
    angleRB.strokeColor = PCyan;
    angleRB.strokeWidth = 0.1 * mm;
    angleRB.filled = false;

    var riskCenter = LI.pathItems.add();
    riskCenter.setEntirePath(Array(Array(4 * mm + radX, WidthForm * mm / 2 + 12 * mm + radY), Array(15 * mm + radX, WidthForm * mm / 2 + 12 * mm + radY)));
    riskCenter.stroked = true;
    riskCenter.strokeColor = PRisk;
    riskCenter.strokeWidth = 0.15 * mm;
    riskCenter.filled = false;
    
    riskCenter = LI.pathItems.add();
    riskCenter.setEntirePath(Array(Array(Rap * mm + 6 * mm + radX, WidthForm * mm / 2 + 12 * mm + radY), Array(Rap * mm - 5 * mm + radX, WidthForm * mm / 2 + 12 * mm + radY)));
    riskCenter.stroked = true;
    riskCenter.strokeColor = PRisk;
    riskCenter.strokeWidth = 0.15 * mm;
    riskCenter.filled = false;

    var i = 0, yRisk = 0, yCompensation = 0;
    var iv = (SAll[0] * mm + Gapp[0] * mm);
    if (casi == 3) { iv /= 2; Stre += 1; }
    
    if (newGroup.height / mm >= (WidthForm - 0.5)) {
        yCompensation = ((newGroup.height / mm - WidthForm) / 2 + 5) * mm;
    }
    
    for (var s1 = 0; s1 <= Stre; s1++) {
        yRisk = WidthLab + i + radY;
        if (yCompensation) {
            s1 == 0 ? yRisk += yCompensation : yRisk;
            s1 == Stre ? yRisk -= yCompensation : yRisk;
        }
        var LabelSw1 = RiskGorizont.pathItems.add();
        LabelSw1.setEntirePath(Array(Array(5 * mm + radX, yRisk), Array(10 * mm + radX, yRisk)));
        LabelSw1.stroked = true;
        LabelSw1.strokeColor = PRisk;
        LabelSw1.strokeWidth = 0.1 * mm;
        LabelSw1.filled = false;

        var LabelSw2 = RiskGorizont.pathItems.add();
        LabelSw2.setEntirePath(Array(Array(Rap * mm + 5 * mm + radX, yRisk), Array(Rap * mm + radX, yRisk)));
        LabelSw2.stroked = true;
        LabelSw2.strokeColor = PRisk;
        LabelSw2.strokeWidth = 0.1 * mm;
        LabelSw2.filled = false;
        i += iv;
    }
    
    var offsetVector = 0 * mm;
    var LVector1 = LI.groupItems.add();
    var LV1 = LVector1.pathItems.add();
    LV1.setEntirePath(Array(Array(4 * mm + radX, WidthLab - 5 * mm + offsetVector + radY), Array(13 * mm + radX, WidthLab - 5 * mm + offsetVector + radY)));
    LV1.stroked = true;
    LV1.strokeColor = PRisk;
    LV1.strokeWidth = 0.1 * mm;
    LV1.filled = false;
    
    var LV2 = LVector1.pathItems.add();
    LV2.setEntirePath(Array(Array(10 * mm + radX, WidthLab - 6 * mm + offsetVector + radY), Array(15 * mm + radX, WidthLab - 5 * mm + offsetVector + radY), Array(10 * mm + radX, WidthLab - 4 * mm + offsetVector + radY)));
    LV2.stroked = true;
    LV2.strokeColor = PRisk;
    LV2.strokeWidth = 0.1 * mm;
    LV2.filled = false;
    
    LVector1.top = Oporka.top - 1 * mm - offsetVector;
    LVector1.left = 11 * mm + radX;
    
    var LVector4 = LVector1.duplicate();
    LVector4.rotate(180);
    LVector4.left = Rap * mm - 12 * mm + radX;
    LVector4.top = Oporka.top - 1 * mm - offsetVector;
    
    var Llog = LI.textFrames.add();
    Llog.position = [(Rap * mm) / 2 - 50 * mm + radX, Oporka.top];
    Llog.contents = "www.justcut.ru  +7(495) 665-20-50";
    Llog.textRange.characterAttributes.size = 13;
    Llog.textRange.characterAttributes.tracking = 30;
    Llog.textRange.characterAttributes.textFont = app.textFonts.getByName(fonts);
    Llog.textRange.characterAttributes.fillColor = PLabel;
    if (Llog.width > Rap * mm) Llog.width = Rap * mm;
    Llog.position = [(Rap * mm) / 2 - (Llog.width / 2) + 5 * mm + radX, Oporka.top - 0.1 * mm];
    
    if (casi != 3) { xEl = SAll[0]; yEl = SAll[1]; }
    
    var LPod = Linfo.textFrames.add();
    var rQ = " | ВАЛ Z " + Math.round(parseFloat(sp.Raport / 3.175)) + " ( " + sp.Raport + " ) ";
    var topPos = 1 * mm;
    var _str = '';
    if (Plosk) rQ = " ";
    if (sp.eWi == 0 || sp.eHi == 0)
        LPod.contents = (" " + Dat + " /  " + Cust + " @  " + parseFloat(parseFloat(xEl).toFixed(2)) + "x " + parseFloat(parseFloat(yEl).toFixed(2)) + rQ + " /  " + Nam + " /  " + Ang + Mate + rll + raa);
    else LPod.contents = (" " + Dat + " /  " + Cust + " @  " + sp.eHi + "x " + sp.eWi + rQ + " /  " + Nam + " /  " + Ang + Mate + rll + raa);
    
    LPod.textRange.characterAttributes.size = 12;
    LPod.textRange.characterAttributes.tracking = 30;
    LPod.textRange.paragraphAttributes.justification = Justification.CENTER;
    LPod.textRange.characterAttributes.textFont = app.textFonts.getByName(fonts);
    LPod.textRange.characterAttributes.fillColor = PLabel;
    
    if (LPod.width > (Rap * mm)) {
        _str = LPod.contents;
        LPod.contents = LPod.contents.replace(/@/g, "\n");
        LPod.contents = LPod.contents.replace(/\|/g, "/");
        topPos = 5.5 * mm;
        if (LPod.width > (Rap * mm)) {
            LPod.contents = _str;
            LPod.contents = LPod.contents.replace(/\|/g, "\n");
            LPod.contents = LPod.contents.replace(/@/g, "/");
            topPos = 10.5 * mm;
        }
    } else LPod.contents = LPod.contents.replace(/@|\|/g, "/");
    LPod.position = [(Rap * mm) / 2 - (LPod.width / 2) + 5 * mm + radX, 16 * mm + topPos + radY];
    
    var LPod2 = Linfo.textFrames.add();
    LPod2.position = [(Rap * mm) / 2 - (LPod2.width / 2) - 38 * mm + radX, 11 * mm + radY];
    LPod2.contents = (sp.data + ' / ' + Distor + " ( " + Lval + " ) -  " + Kni + " -  " + Ang + " - Mirror, L " + lineOb + dop);
    LPod2.textRange.characterAttributes.size = 12;
    LPod2.textRange.characterAttributes.tracking = 30;
    LPod2.textRange.characterAttributes.alignment = StyleRunAlignmentType.center;
    LPod2.textRange.characterAttributes.textFont = app.textFonts.getByName(fonts);
    LPod2.textRange.characterAttributes.fillColor = PLabel;
    if (LPod2.width > Rap * mm) LPod2.width = Rap * mm;
    LPod2.position = [(Rap * mm) / 2 - (LPod2.width / 2) + 5 * mm + radX, 11.3 * mm + radY];
    
    var bigNamber = NambeL.textFrames.add();
    bigNamber.position = [(Rap * mm / 2 - 60 * mm + radX), (Oporka.top - 12 * mm) / 2 + radY / 2];
    bigNamber.contents = Nam;
    bigNamber.textRange.characterAttributes.size = 100;
    bigNamber.textRange.characterAttributes.alignment = StyleRunAlignmentType.center;
    bigNamber.textRange.characterAttributes.textFont = app.textFonts.getByName(fonts);
    bigNamber.textRange.characterAttributes.fillColor = PFull;
    
    if (Distor != 0) {
        LI.width *= Distor;
        newGroup.width *= Distor;
        if (sp.offset != '0') {
            newGroup.left = (5 * mm + parseFloat(sp.offset) * mm) * Distor - lineOb * mm / 2.0 + radX;
        } else {
            newGroup.left = 5 * mm + (LI.width - newGroup.width - 6 * mm) / 2 - lineOb * mm / 2.0 + radX;
        }
    }
    
    {
        newGroup.top = WidthForm * mm / 2.0 + newGroup.height / 2.0 + lineOb * mm / 2.0 + 12 * mm + radY;
        RiskGorizont.top = WidthForm * mm / 2.0 + RiskGorizont.height / 2.0 + 0.15 * mm / 2.0 + 12 * mm + radY;
        if (Plosk) newGroup.left = 5 * mm + (LI.width - newGroup.width - 6 * mm) / 2 - lineOb * mm / 2.0 + radX;
    }
    
    risk(LI.width);
    riskDop(LI.width, (WidthForm - newGroup.height / mm).toFixed(2));
    
    //-------------------------REZ---------------------------------
    var lineCenter = Linfo.pathItems.add();
    lineCenter.setEntirePath(Array(Array(Oporka.left - 0.55 * mm + Oporka.strokeWidth / 2, WidthForm * mm / 2 + 12 * mm + radY), Array(Oporka.left + Oporka.width + 0.55 * mm + Oporka.strokeWidth / 2, WidthForm * mm / 2 + 12 * mm + radY)));
    lineCenter.stroked = false;
    lineCenter.strokeColor = Prll;
    lineCenter.strokeWidth = 0.01 * mm;
    lineCenter.filled = false;
    lineCenter.strokeOverprint = true;
    
    var leftRez = NambeL.pathItems.add();
    leftRez.setEntirePath(Array(Array(Oporka.left + 0.3 * mm + Oporka.strokeWidth / 2, WidthForm * mm + 15 * mm + radY), Array(Oporka.left + 0.3 * mm + Oporka.strokeWidth / 2, 9 * mm + radY)));
    leftRez.stroked = false;
    leftRez.strokeColor = Prll;
    leftRez.strokeWidth = 0.01 * mm;
    leftRez.filled = false;
    leftRez.strokeOverprint = true;

    var rightRez = NambeL.pathItems.add();
    rightRez.setEntirePath(Array(Array(Oporka.left + Oporka.width - 0.3 * mm + Oporka.strokeWidth / 2, WidthForm * mm + 15 * mm + radY), Array(Oporka.left + Oporka.width - 0.3 * mm + Oporka.strokeWidth / 2, 9 * mm + radY)));
    rightRez.stroked = false;
    rightRez.strokeColor = Prll;
    rightRez.strokeWidth = 0.01 * mm;
    rightRez.filled = false;
    rightRez.strokeOverprint = true;
    
    // === Прямоугольник на слое REZ получает офсет +radX ===
    var RezR = REZ.pathItems.roundedRectangle((12 + 0.3) * mm + radY, Oporka.left + 0.3 * mm + Oporka.strokeWidth / 2, (Rap * Distor - 0.6) * mm, (-WidthForm + 0.6) * mm, 3 * mm, 3 * mm);
    RezR.closed = true;
    RezR.filled = false;
    RezR.stroked = false;
    RezR.strokeColor = Prll;
    RezR.strokeWidth = 0.01 * mm;
    RezR.strokeOverprint = true;
	
	if (sp.rad) {
		
	var RezROffset = REZ.pathItems.roundedRectangle(0, 0, (Rap * Distor - 0.6) * mm + (2*ofst), (-WidthForm + 0.6) * mm - (2*ofst), 3 * mm, 3 * mm);
		RezROffset.top = RezR.top + ofst;
		RezROffset.left = RezR.left - ofst;
		RezROffset.closed = true;
		RezROffset.filled = false;
		RezROffset.stroked = false;
		RezROffset.strokeColor = Prll;
		RezROffset.strokeWidth = 0.01 * mm;
		RezROffset.strokeOverprint = true;
	}
    
    // === Риски на слое REZ (1 мм) НЕ ТРОГАЕМ ===
    var Rez0 = REZ.pathItems.add();
    Rez0.setEntirePath(Array(Array(0, 0), Array(0, mm)));
    Rez0.stroked = false;
    Rez0.strokeColor = Prll;
    Rez0.strokeWidth = 0.01 * mm;
    Rez0.filled = false;
    Rez0.strokeOverprint = true;
    
    var Rez1 = REZ.pathItems.add();
    Rez1.setEntirePath(Array(Array(Rap * mm, 0), Array(Rap * mm, mm)));
    Rez1.stroked = true;
    Rez1.strokeColor = Prll;
    Rez1.strokeWidth = 0.01 * mm;
    Rez1.filled = false;
    Rez1.strokeOverprint = true;
    
    //------------------POINT (ОСТАВЛЕНЫ НА МЕСТЕ)----------------------------------------
    var Point0 = L_Test.groupItems.add();
    var P0K = Point0.pathItems.ellipse(dotPoint / 2, -dotPoint / 2, dotPoint, dotPoint, false, false);
    P0K.stroked = false;
    P0K.filled = true;
    P0K.fillColor = PFull;
    P0K.fillOverprint = false;
    var P0W = Point0.pathItems.ellipse(dotPoint / 8, -dotPoint / 8, dotPoint / 4, dotPoint / 4, false, false);
    P0W.stroked = false;
    P0W.filled = true;
    P0W.fillColor = PWhite;
    P0W.fillOverprint = false;
    var P1K = Point0.duplicate();
    P1K.left += Rap * mm;
	
    
    //----------RLL (ОСТАВЛЕНЫ НА МЕСТЕ)----------
    var rll0 = newLayer.pathItems.add();
    rll0.setEntirePath(Array(Array(0, 0), Array(0, -mm)));
    rll0.stroked = false;
    rll0.strokeColor = Prll;
    rll0.strokeWidth = 0.01 * mm;
    rll0.filled = false;
    rll0.strokeOverprint = true;

    var rll1 = newLayer.pathItems.add();
    rll1.setEntirePath(Array(Array(Rap * mm, 0), Array(Rap * mm, -mm)));
    rll1.stroked = true;
    rll1.strokeColor = Prll;
    rll1.strokeWidth = 0.01 * mm;
    rll1.filled = false;
    rll1.strokeOverprint = true;
	
	if (sp.rad) {
		var P2K = Point0.duplicate();
			P2K.top = Point0.top + ofst;
		var P3K = P1K.duplicate();
			P3K.top = P1K.top + ofst;
		
		var r2l0 = rll0.duplicate();
			r2l0.top = rll0.top + ofst;
		var r2l1 = rll1.duplicate();
			r2l1.top = rll1.top + ofst;
			
		var Rez2 = Rez0.duplicate();
			Rez2.top = Rez0.top + ofst;
		var Rez3 = Rez1.duplicate();
			Rez3.top = Rez1.top + ofst;
	}
    
    var countLabel = 0;
    var i_ruler = 0;
    var gor = parseInt((Rap) / 10);
    if (gor > 40) gor = 40;
    var ver = parseInt(WidthForm / 10);
    if (ver > 40) ver = 40;
    
    // === L-TEST И ЧИСЛА НЕ ТРОГАЕМ ===
    for (var s = 0; s < gor; s++) {
        countLabel++;
        var LTest = L_Test.groupItems.add();
        var newPath = LTest.pathItems.add();
        newPath.setEntirePath(Array(Array(5 * mm + i_ruler, 6 * mm), Array(5 * mm + i_ruler, 0), Array(8.5 * mm + i_ruler, 0)));
        newPath.stroked = true;
        newPath.strokeColor = PFull;
        newPath.strokeCap = StrokeCap.ROUNDENDCAP;
        newPath.strokeWidth = lineOb * mm;
        newPath.filled = false;
        var LText = Linfo.textFrames.add();
        LText.position = [7 * mm + i_ruler, 6 * mm];
        LText.contents = countLabel;
        LText.textRange.characterAttributes.size = 11;
        LText.textRange.characterAttributes.textFont = app.textFonts.getByName(fonts);
        LText.textRange.characterAttributes.fillColor = PLabel;
        i_ruler += 10 * mm;
    }
    
    countLabel = 0;
    i_ruler = 0;
    for (var sv = 0; sv < ver; sv++) {
        countLabel++;
        LTest = L_Test.groupItems.add();
        newPath = LTest.pathItems.add();
        newPath.setEntirePath(Array(Array(-8 * mm, 6 * mm + i_ruler), Array(-8 * mm, i_ruler), Array(-4.5 * mm, i_ruler)));
        newPath.stroked = true;
        newPath.strokeColor = PFull;
        newPath.strokeCap = StrokeCap.ROUNDENDCAP;
        newPath.strokeWidth = lineOb * mm;
        newPath.filled = false;
        LText = Linfo.textFrames.add();
        LText.position = [-6 * mm, 6 * mm + i_ruler];
        LText.contents = countLabel;
        LText.textRange.characterAttributes.size = 11;
        LText.textRange.characterAttributes.textFont = app.textFonts.getByName(fonts);
        LText.textRange.characterAttributes.fillColor = PLabel;
        i_ruler += 10 * mm;
    }
    
	if (sp.rll == 'RLL') {
        runExpDXF(Nams, Polurot || Rap, mm, irll);
        //return;
    }
	
    // === Gabarit: остался на месте, но расширился вправо и вверх на radX ===
    var Gabarit = L_Test.pathItems.rectangle(-5 * mm, -12 * mm, Rap * mm + 23 * mm + ofst, -WidthForm * mm - 27 * mm - ofst);
    Gabarit.closed = true;
    Gabarit.filled = false;
    Gabarit.stroked = true;
    Gabarit.strokeColor = PFull;
    Gabarit.fillOverprint = false;
    Gabarit.strokeWidth = 1.5 * mm; 
    
    newLayer = null;
    Linfo = null;
    L_Test = null;

    //---------------------------------- functions
    function circ() {
        var i = 0;
        var j = 0;
        var elip = elm.pathItems.ellipse(0, 0, 70, 70, false, false);
        elip.stroked = true;
        elip.filled = false;
        elip.strokeColor = PFull;
        elip.strokeWidth = lineOb * mm;
        elip.width = SAll[0] * mm;
        elip.height = SAll[1] * mm;
        elip.top = WidthLab + i + elip.height + Gapp[0] * mm / 2 + lineOb * mm / 2.0 + radY;
        elip.left = (5 * mm - lineOb * mm / 2.0) + ((Gap2) * mm / 2) + radX;
    }

    function rect() {
        var i = 0;
        var j = 0;
        var elip = elm.pathItems.roundedRectangle(0, 0, SAll[0] * mm, SAll[1] * mm, SAll[2] * mm, SAll[2] * mm, false);
        elip.stroked = true;
        elip.filled = false;
        elip.strokeColor = PFull;
        elip.strokeWidth = lineOb * mm;
        elip.top = WidthLab + i + elip.height + Gapp[0] * mm / 2 + lineOb * mm / 2 + radY;
        elip.left = (5 * mm - lineOb * mm / 2) + ((Gap2) * mm / 2) + radX;
    }

    function el() {
        if (SAll[0] == "true") selAll(); else {
            if (app.documents.length > 0) {
                docSelection2 = docRef.selection;
                if (docSelection2.length > 0) {
                    for (var i = 0; i < docSelection2.length; i++) {
                        docSelection = docSelection2[i];
                        docSelection.strokeColor = PFull;
                        docSelection.filled = false;
                        docSelection.strokeWidth = lineOb * mm;
                        docSelection.moveToBeginning(elm);
                    }
                    SAll[0] = elm.width / mm; SAll[1] = elm.height / mm;
                } else alert("No selection");
            }
        }
    }

    function selAll() {
        var iSel = 0;
        for (var i = 0; i < documents[0].pathItems.length; i++) {
            docSelection = documents[0].pathItems[i];
            docSelection.strokeColor = PFull;
            docSelection.filled = false;
            docSelection.strokeWidth = lineOb * mm;
            docSelection.moveToBeginning(elm);
            iSel += documents[0].pathItems[i].length / mm;
        }
        SAll[0] = elm.width / mm; SAll[1] = elm.height / mm;
        xEl = documents[0].pathItems[0].width / mm; yEl = documents[0].pathItems[0].height / mm;
    }

    function elSelect(mirror) {
        if (mirror) { elm.transform(app.getScaleMatrix(-100, 100)); elm.rotate(-90); }
        else elm.rotate(90);
        var i = 0;
        var j = 0;
        elm.strokeColor = PFull;
        elm.filled = false;
        elm.strokeWidth = lineOb * mm;
        elm.top = WidthLab + i + elm.height + Gapp[0] * mm / 2 + lineOb * mm / 2 + radY;
        elm.left = (5 * mm - lineOb * mm / 2) + ((Gap2) * mm / 2) + j + radX;
        var kostyl = false;

        var tr = Gap2 * mm + SAll[1] * mm;
        for (var q = 0; q < Rep; q++) {
            for (var w = 0; w < Stre; w++) {
                if (kostyl) elm.duplicate();
                elm.strokeColor = PFull;
                elm.filled = false;
                elm.strokeWidth = lineOb * mm;
                elm.top = WidthLab + i + elm.height + Gapp[0] * mm / 2 + lineOb * mm / 2 + radY;
                elm.left = (5 * mm - lineOb * mm / 2) + ((Gap2) * mm / 2) + j + radX;
                i += (SAll[0] * mm + Gapp[0] * mm);
                kostyl = true;
            }
            j += tr;
            i = 0;
        }
    }

    function fullSelect() {
        elm.transform(app.getScaleMatrix(-100, 100));
        elm.rotate(-90);
        elm.strokeColor = PFull;
        elm.filled = false;
        elm.strokeWidth = lineOb * mm;
        elm.top = WidthLab + elm.height + lineOb * mm / 2 + Gapp[0] * mm / 2 + radY;
        elm.left = (5 * mm - lineOb * mm / 2) + ((PolurotY - SAll[1]) * mm / 2) + radX;
        Rep = 1;
        Stre = 1;
    }

    function closePath() {
        var warning_limit = 400;
        var X = "Close All Paths\n\n";
        var MSG_asksel = X + "Close all selected open path-items? ";
        var MSG_ask = X + "Close all open path-items in this document. ";
        var MSG_allclosed = X + "There are no open path-items in this document. ";
        var MSG_allselclosed = X + "The selection does not contain any open path-item. ";
        var MSG_nopath = X + "You have not selected any path-item. ";
        var MSG_nodocs = X + "You have no open document. ";
        var MSG_warning = X + "The document will be analyzed for open path-items ... ";
        var MSG_status = X + "Closed path-items:  ";
        
        var error = 0;
        var locked = false;
        var proccessedItems = 0;
        if (documents.length < 1) {
            error++;
            alert(MSG_nodocs);
        } else {
            var allPaths = activeDocument.pathItems;
            if (selection.length > 0) {
                var onlySelection = 1;
                var selcount = 0;
                for (var i = 0; i < selection.length; i++) {
                    if (selection[i].typename == "PathItem") { selcount++; }
                }
                if (selcount == selection.length) {
                    var allPaths = selection;
                }
            } else {
                var onlySelection = 0;
            }
            if (allPaths.length > warning_limit) { alert(MSG_warning + " ( " + allPaths.length + " ) "); }
            var count = 0;
            for (var i = 0; i < allPaths.length; i++) {
                locked = false;
                isLocked(allPaths[i]);
                if ((allPaths[i].selected == true || onlySelection == 0) && !locked && !allPaths[i].layer.locked) {
                    if (!allPaths[i].closed) { count++; }
                }
            }
            if (count == 0) {
                error++;
                if (selection.length > 0) { alert(MSG_allselclosed); } else { alert(MSG_allclosed); }
            }
            if (allPaths.length < 0) { error++; alert(MSG_nopath); }
            if (error < 1) {
                if (selection.length > 0) {
                    var confirmed = confirm(MSG_asksel + " (" + count + ")");
                } else {
                    var confirmed = confirm(MSG_ask + " (" + count + ")");
                }
                if (confirmed) {
                    close(allPaths);
                    if (proccessedItems != count) {
                        alert(MSG_status + proccessedItems + "/" + count);
                    } else {
                        alert(MSG_status + proccessedItems);
                    }
                }
            }
        }
    }

    function close(thePaths) {
        for (var i = 0; i < thePaths.length; i++) {
            if ((thePaths[i].selected == true || onlySelection == 0) && !thePaths[i].closed) {
                try {
                    thePaths[i].closed = true;
                    proccessedItems++;
                } catch (e) { }
            }
        }
    }

    function isLocked(test) {
        if (test.typename == "Layer") {
            if (!locked) { locked = test.locked; }
            if (!locked) { locked = test.hidden; }
        } else {
            if (test.typename != "Layer" && test.locked == true) {
                if (!locked) { locked = test.locked; }
                if (!locked) { locked = test.hidden; }
            } else {
                isLocked(test.parent);
            }
        }
    }

    function risk(li_wi) {
        var LabelW2 = LI.pathItems.add();
        LabelW2.setEntirePath(Array(Array(newGroup.left + lineOb * mm / 2, 11 * mm + radY), Array(newGroup.left + lineOb * mm / 2, 17 * mm + radY)));
        LabelW2.stroked = true;
        LabelW2.strokeColor = PRisk;
        LabelW2.strokeWidth = 0.1 * mm;
        LabelW2.filled = false;
        
        LabelW2 = LI.pathItems.add();
        LabelW2.setEntirePath(Array(Array(newGroup.left + newGroup.width + lineOb * mm / 2, 11 * mm + radY), Array(newGroup.left + newGroup.width + lineOb * mm / 2, 17 * mm + radY)));
        LabelW2.stroked = true;
        LabelW2.strokeColor = PRisk;
        LabelW2.strokeWidth = 0.1 * mm;
        LabelW2.filled = false;
        
        LabelW2 = LI.pathItems.add();
        LabelW2.setEntirePath(Array(Array(newGroup.left + lineOb * mm / 2, WidthForm * mm + 13 * mm + radY), Array(newGroup.left + lineOb * mm / 2, WidthForm * mm + 7 * mm + radY)));
        LabelW2.stroked = true;
        LabelW2.strokeColor = PRisk;
        LabelW2.strokeWidth = 0.1 * mm;
        LabelW2.filled = false;
        
        LabelW2 = LI.pathItems.add();
        LabelW2.setEntirePath(Array(Array(newGroup.left + newGroup.width + lineOb * mm / 2, WidthForm * mm + 13 * mm + radY), Array(newGroup.left + newGroup.width + lineOb * mm / 2, WidthForm * mm + 7 * mm + radY)));
        LabelW2.stroked = true;
        LabelW2.strokeColor = PRisk;
        LabelW2.strokeWidth = 0.1 * mm;
        LabelW2.filled = false;
        
        var LabelSw1 = LI.pathItems.add();
        LabelSw1.setEntirePath(Array(Array(4 * mm + radX, newGroup.top - lineOb * mm / 2), Array(9 * mm + radX, newGroup.top - lineOb * mm / 2)));
        LabelSw1.stroked = true;
        LabelSw1.strokeColor = PRisk;
        LabelSw1.strokeWidth = 0.1 * mm;
        LabelSw1.filled = false;
        
        LabelSw1 = LI.pathItems.add();
        LabelSw1.setEntirePath(Array(Array(4 * mm + radX, newGroup.top - newGroup.height - lineOb * mm / 2), Array(9 * mm + radX, newGroup.top - newGroup.height - lineOb * mm / 2)));
        LabelSw1.stroked = true;
        LabelSw1.strokeColor = PRisk;
        LabelSw1.strokeWidth = 0.1 * mm;
        LabelSw1.filled = false;
        
        var LabelSw2 = LI.pathItems.add();
        LabelSw2.setEntirePath(Array(Array(li_wi + radX, newGroup.top - lineOb * mm / 2), Array(li_wi - 5 * mm + radX, newGroup.top - lineOb * mm / 2)));
        LabelSw2.stroked = true;
        LabelSw2.strokeColor = PRisk;
        LabelSw2.strokeWidth = 0.1 * mm;
        LabelSw2.filled = false;
        
        LabelSw2 = LI.pathItems.add();
        LabelSw2.setEntirePath(Array(Array(li_wi + radX, newGroup.top - newGroup.height - lineOb * mm / 2), Array(li_wi - 5 * mm + radX, newGroup.top - newGroup.height - lineOb * mm / 2)));
        LabelSw2.stroked = true;
        LabelSw2.strokeColor = PRisk;
        LabelSw2.strokeWidth = 0.1 * mm;
        LabelSw2.filled = false;
    }

    function riskDop(li_wi, bigRisk) {
        var LabelSw1 = LI.pathItems.add();
        LabelSw1.setEntirePath(Array(Array(4 * mm + radX, newGroup.top - lineOb * mm / 2 + bigRisk * mm / 4), Array(15 * mm + radX, newGroup.top - lineOb * mm / 2 + bigRisk * mm / 4)));
        LabelSw1.stroked = true;
        LabelSw1.strokeColor = PRisk;
        LabelSw1.strokeWidth = 0.15 * mm;
        LabelSw1.filled = false;
        
        LabelSw1 = LI.pathItems.add();
        LabelSw1.setEntirePath(Array(Array(4 * mm + radX, newGroup.top - newGroup.height - lineOb * mm / 2 - bigRisk * mm / 4), Array(15 * mm + radX, newGroup.top - newGroup.height - lineOb * mm / 2 - bigRisk * mm / 4)));
        LabelSw1.stroked = true;
        LabelSw1.strokeColor = PRisk;
        LabelSw1.strokeWidth = 0.15 * mm;
        LabelSw1.filled = false;
        
        var LabelSw2 = LI.pathItems.add();
        LabelSw2.setEntirePath(Array(Array(li_wi + radX, newGroup.top - lineOb * mm / 2 + bigRisk * mm / 4), Array(li_wi - 11 * mm + radX, newGroup.top - lineOb * mm / 2 + bigRisk * mm / 4)));
        LabelSw2.stroked = true;
        LabelSw2.strokeColor = PRisk;
        LabelSw2.strokeWidth = 0.15 * mm;
        LabelSw2.filled = false;
        
        LabelSw2 = LI.pathItems.add();
        LabelSw2.setEntirePath(Array(Array(li_wi + radX, newGroup.top - newGroup.height - lineOb * mm / 2 - bigRisk * mm / 4), Array(li_wi - 11 * mm + radX, newGroup.top - newGroup.height - lineOb * mm / 2 - bigRisk * mm / 4)));
        LabelSw2.stroked = true;
        LabelSw2.strokeColor = PRisk;
        LabelSw2.strokeWidth = 0.15 * mm;
        LabelSw2.filled = false;
    }

    function recRap(Rap, WidthForm) {
        var Pr = new CMYKColor();
        Pr.name = 'PrColor';
        Pr.black = 0; Pr.cyan = 100; Pr.magenta = 0; Pr.yellow = 100;
        
        var RRap = Linfo.pathItems.rectangle(12 * mm + radX, 5 * mm + radX, Rap * mm, -WidthForm * mm);
        RRap.closed = true;
        RRap.filled = false;
        RRap.stroked = true;
        RRap.strokeColor = Pr;
        RRap.fillOverprint = false;
        RRap.strokeWidth = 0.3 * mm;
        
        var RR1 = REZ.pathItems.rectangle(-3 * mm + radX, -10 * mm + radX, Rap * mm + 30 * mm, -WidthForm * mm - 30 * mm);
        RR1.closed = true;
        RR1.filled = false;
        RR1.stroked = true;
        RR1.strokeColor = Pr;
        RR1.fillOverprint = false;
        RR1.strokeWidth = 0;
        
        app.activeDocument.layers["REZ"].hasSelectedArtwork = true;
        app.activeDocument.fitArtboardToSelectedArt(0);
        app.activeDocument.selection = null;
        app.activeDocument.layers.getByName('L-Test').remove();
        app.activeDocument.layers.getByName('REZ').remove();
    }

    try {
        PMagenta = PCyan = PLabel = PRisk = PWhite = PFull = Prll = null;
        newLayer = Linfo = L_Test = REZ = NambeL = newGroup = elm = null;
        Oporka = LI = RiskGorizont = LVector1 = LVector4 = null;
        Llog = LPod = LPod2 = bigNamber = null;
        lineCenter = leftRez = rightRez = RezR = Rez0 = Rez1 = null;
        Point0 = P0K = P0W = P1K = rll0 = rll1 = null;
        Gabarit = null;
        docRef = null;
    } catch (e) {}
    
    $.gc();
    return;
}

function runExpDXF(Nams, Rap, mm, irll) {
    #include './expDXF.js';
    $.gc();
}